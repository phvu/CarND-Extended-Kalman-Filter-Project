...
def telemetry(sid, data):
	if data:
		...
		rmse_idx = next((i for i in range(len(outputVals)) if outputVals[i].find("RMSE") != -1), -1)
		if rmse_idx >= 0:
			send_estimate_rmse(x_markers,y_markers, outputVals[rmse_idx + 1],
							   outputVals[rmse_idx + 2],outputVals[rmse_idx + 3],outputVals[rmse_idx + 4])
		else:	
